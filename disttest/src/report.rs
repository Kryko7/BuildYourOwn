//! Terminal output (the same shape as `shelltest` and `kafkatest`) and the `--json` report.
//!
//! The JSON schema is the one the other testers write, field for field, so `byo` and the
//! site ingest a `disttest` report with no branch of their own. The only thing that carries
//! track-specific information is each test's `tags`, which is where the ladder lives.
...
pub struct Reporter {
    /// True when the run is a suite self-check against the ladders' references.
    pub validate: bool,
...
impl Reporter {
    ...
    pub fn stage_header(&self, st: &Stage) {
        self.out(&format!(
            ...
            c(st.name, Tint::Cyan),
            c(&format!("[{}]", st.ladder.as_str()), Tint::Dim)
        ));
    }
    ...
    /// Print a node's captured output under a failing test.
    pub fn node_output(&self, text: &str) {
        if text.trim().is_empty() {
            return;
        }
        self.section("node output");
        self.row(text);
    }
    ...
    pub fn total(
        ...
        if failed > 0 && self.validate {
            self.out(&c(
                &format!(
                    "SUITE BUGS: the reference for '{target}' failed {failed} test(s). \
                     Fix the expectations, not the reference:"
                ),
                Tint::Red,
            ));
            for (st, results) in all {
    ...
    /// One line per ladder: how many of its tests ran and how many passed.
    fn ladder_totals(&self, all: &[(&Stage, Vec<TestResult>)]) {
        for ladder in crate::stages::Ladder::ALL {
            let mut passed = 0;
            let mut failed = 0;
            let mut skipped = 0;
            let mut stages = 0;
            for (st, results) in all.iter().filter(|(s, _)| s.ladder == ladder) {
                let _ = st;
                stages += 1;
                let (p, f, s) = counts(results);
                passed += p;
                failed += f;
                skipped += s;
            }
            ...
            let ran = passed + failed;
            let line = format!(
                "  {:<11} {stages:>2} stages  {passed}/{ran} passed{}",
                ladder.as_str(),
                if skipped > 0 {
...
/// Top-level `--json` document; the schema `shelltest` and `kafkatest` already write.
#[derive(Serialize)]
...
mod tests {
    ...
    fn result(name: &str, status: Status, ext: bool) -> TestResult {
        TestResult {
            name: name.into(),
            status,
            ext,
    ...
    fn json_uses_target_and_counts_correctly() {
        let st = stage();
        let mut bad = result("bad", Status::Fail, true);
        bad.failure = Some(Failure::new(FailureKind::NodeCrash, "boom"));
        let mut ok = result("ok", Status::Pass, false);
        ok.notes.push("p99 1.2 ms".into());
        ...
        assert_eq!(v["target"], "etcd");
        assert_eq!(v["validate"], true);
        assert_eq!(v["passed"], 1);
        assert_eq!(v["failed"], 1);
        assert_eq!(v["stages"][0]["file"], "src/stages/s22_put_header.rs");
        assert_eq!(v["stages"][0]["tests"][1]["status"], "fail");
        assert_eq!(v["stages"][0]["tests"][1]["failure_kind"], "node_crash");
        assert_eq!(v["stages"][0]["tests"][1]["failures"][0], "boom");
        assert_eq!(v["stages"][0]["tests"][0]["tags"][0], "node");
        assert!(v.get("shell").is_none(), "disttest reports `target`");
        assert!(
            v.get("ladders").is_none(),
            "the ladder lives in each test's tags, not at the top level"
        );
        assert_eq!(v["stages"][0]["tests"][0]["notes"][0], "p99 1.2 ms");
    }
    ...
    fn counts_split_pass_fail_skip() {
        assert_eq!(
            counts(&[
                result("a", Status::Pass, false),
                result("b", Status::Fail, false),
                result("c", Status::Skip, false),
                result("d", Status::Pass, false),
            ]),
            (2, 1, 1)
        );
    }
