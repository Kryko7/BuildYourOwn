//! Terminal output (same shape as `shelltest`) and the `--json` report.
//!
//! The JSON schema is the one `shelltest` writes, with `target` in place of `shell`, so the
//! site can read both tracks with one parser. `failure_kind` is the only addition: it tells
//! a crashed broker apart from a wrong byte.
...
pub struct Reporter {
    /// True when the run is a suite self-check against the reference broker.
    pub validate: bool,
...
impl Reporter {
    ...
    fn failure_block(&self, f: &Failure) {
        ...
        for (title, dump) in f.hex_blocks() {
            self.section(&title);
            self.row(&dump);
        }
    }

    /// Print the broker's captured output under a failing test.
    pub fn broker_output(&self, text: &str) {
        if text.trim().is_empty() {
            return;
        }
        self.section("broker output");
        self.row(text);
    }
    ...
    pub fn total(
        ...
        if failed > 0 && self.validate {
            self.out(&c(
                &format!(
                    "SUITE BUGS: the reference broker '{target}' failed {failed} test(s). \
                     Fix the expectations, not the broker:"
                ),
                Tint::Red,
            ));
            for (st, results) in all {
...
/// Top-level `--json` document; the same shape as `shelltest`'s, with `target` for `shell`.
#[derive(Serialize)]
pub struct JsonReport<'a> {
    /// The broker that was tested.
    pub target: &'a str,
    ...
    fn json_uses_target_and_counts_correctly() {
        ...
        let results = vec![
            ...
            TestResult {
                ...
                ext: true,
                duration_ms: 2,
                failure: Some(Failure::new(FailureKind::BrokerCrash, "boom")),
                skip_reason: None,
                notes: Vec::new(),
            },
        ];
        ...
        assert_eq!(v["target"], "apache_kafka");
        assert_eq!(v["validate"], true);
        assert_eq!(v["passed"], 1);
        assert_eq!(v["failed"], 1);
        assert_eq!(v["stages"][0]["file"], "src/stages/s03_request_header.rs");
        assert_eq!(v["stages"][0]["tests"][1]["status"], "fail");
        assert_eq!(v["stages"][0]["tests"][1]["failure_kind"], "broker_crash");
