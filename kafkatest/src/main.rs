...
use kafkatest::stages::Stage;
use kafkatest::{catalog, config, examples, report, runner, stages};
use std::path::{Path, PathBuf};
use std::time::Instant;

/// stage-by-stage black-box tester for Kafka brokers.
#[derive(Parser, Debug)]
#[command(name = "kafkatest", version, about)]
struct Cli {
    /// Broker to test: a name from brokers.yaml (apache_kafka, my_broker, ...) or a path
    #[arg(long)]
    ...
    #[arg(long, short)]
    verbose: bool,
    /// Keep each broker instance's temporary directory
    #[arg(long)]
    keep_tmp: bool,
    /// Per-test timeout in milliseconds (broker boot is not counted)
    #[arg(long, value_name = "N", default_value_t = 10_000)]
    timeout_ms: u64,
    /// Run everything against the reference broker and word failures as suite bugs
    #[arg(long)]
    validate: bool,
    ...
    #[arg(long)]
    no_color: bool,
    /// Path to brokers.yaml
    #[arg(long, value_name = "FILE")]
    brokers_file: Option<PathBuf>,
    /// Force the broker's listener port (never 9092 for the reference broker)
    #[arg(long, value_name = "PORT")]
    port: Option<u16>,
    /// Force the broker's log directory
    #[arg(long, value_name = "DIR")]
    log_dir: Option<PathBuf>,
    /// Seed for every random choice, so a run can be reproduced
    #[arg(long, default_value_t = 0x5eed)]
    seed: u64,
    /// Send every stage's examples to the broker and record the answers into this file
    #[arg(long, value_name = "FILE")]
...
fn locate(explicit: Option<PathBuf>, relative: &str) -> Result<PathBuf> {
    ...
    roots
        ...
        .find(|p| p.exists())
        .ok_or_else(|| anyhow::anyhow!("cannot find {relative}; pass --brokers-file"))
}

/// The stage range a command line asks for.
///
/// `--validate` only changes the broker and the wording of a failure, never the selection:
...
fn run() -> Result<bool> {
    ...
    let brokers_file = locate(cli.brokers_file.clone(), "brokers.yaml")?;
    let brokers = config::load_brokers(&brokers_file)?;
    let spec = cli
        .broker
        .as_deref()
        .context("--broker <name|path> is required")?;
    let def = config::resolve_broker(spec, &brokers)?;
    if cli.validate && !brokers.contains_key(spec) {
        bail!(
            "--validate needs a registered reference broker (one of: {})",
            brokers.keys().cloned().collect::<Vec<_>>().join(", ")
        );
    }
    if cli.port == Some(9092) && def.kind == config::BrokerKind::Reference {
        bail!("the reference broker must not be pinned to port 9092");
    }
    ...
    if let Some(out) = cli.capture_examples.clone() {
        ...
        println!(
            "kafkatest: capturing examples from '{}' into {}",
            def.name,
            out.display()
        );
    ...
    rt.block_on(async move {
        println!(
            "kafkatest: testing '{}' with {} (seed {:#x})",
            def.name,
            brokers_file.display(),
            cli.seed
        );
        ...
        for st in &picked {
            ...
            if let Err(f) = runner.begin_stage(st).await {
                bail!(
                    "cannot start the broker for stage {}: {}",
                    st.number,
                    f.messages.join("; ")
                );
            }
            ...
            for t in tests {
                ...
                if failed {
                    if let Some(out) = runner.broker_output() {
                        reporter.broker_output(&out);
                    }
                }
                results.push(r);
            }
...
mod tests {
    ...
    fn validate_changes_the_wording_not_the_selection() {
        ...
        assert!(
            range(&cli(&["--broker", "apache_kafka"])).is_err(),
