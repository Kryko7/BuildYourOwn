...
pub struct CatalogTest {
    ...
    pub tags: Vec<String>,
    /// Brokers this test is skipped for.
...
pub struct CatalogStage {
    ...
    /// Worked examples: request bytes, response bytes and per-field annotations, captured
    /// from the reference broker by `--capture-examples`.
...
pub fn list(stages: &[Stage], plan: &Path) {
    ...
    for st in stages {
        ...
        println!(
            "{tick} Stage {:02}  {:<46} {:<38} {} tests{ext_note}, {} examples",
            st.number,
            st.name,
            st.file_name(),
            st.tests.len(),
