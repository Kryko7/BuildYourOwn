//! Terminal output (the same shape `shelltest` and `kafkatest` print) and the `--json`
//! report.
//!
//! The JSON schema is the one those two write, so the site and `byo` read all three tracks
//! with one parser: `target`, `validate`, `stages[]`, and per test `status`, `ext`,
//! `duration_ms`, `failures`, `actual`, `failure_kind`, `notes`.
...
pub struct Reporter {
    /// True when the run is a suite self-check against the reference runtime.
    pub validate: bool,
...
impl Reporter {
    ...
    fn failure_block(&self, f: &Failure) {
        ...
        for (title, body) in f.detail_blocks() {
            self.section(&title);
            self.row(&body);
        }
    }
    ...
    pub fn total(
        ...
        if failed > 0 && self.validate {
            self.out(&c(
                &format!(
                    "SUITE BUGS: the reference runtime '{target}' failed {failed} test(s). \
                     Fix the expectations, not the runtime:"
                ),
                Tint::Red,
            ));
            for (st, results) in all {
...
/// Top-level `--json` document; the same shape `shelltest` and `kafkatest` write.
#[derive(Serialize)]
pub struct JsonReport<'a> {
    /// The runtime that was tested.
    pub target: &'a str,
...
pub struct JsonTest {
    ...
    /// Labelled values that were seen (path → value).
    ...
    fn json_uses_target_and_counts_correctly() {
        ...
        let results = vec![
            TestResult {
                ...
                failure: None,
                skip_reason: None,
                notes: vec!["500 mutations, 0 crashes".into()],
            },
            TestResult {
                name: "bad".into(),
                status: Status::Fail,
                ext: true,
                duration_ms: 2,
                failure: Some(Failure::new(FailureKind::RuntimeCrash, "boom")),
                skip_reason: None,
                notes: Vec::new(),
            },
        ];
