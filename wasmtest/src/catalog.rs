...
pub struct CatalogTest {
    ...
    pub tags: Vec<String>,
    /// Runtimes this test is skipped for.
...
pub struct CatalogStage {
    ...
    /// Worked examples: the module's annotated bytes and what a correct runtime prints.
...
///
/// Unlike the Kafka track there is nothing to capture first: the examples are modules this
/// crate encodes, so `--list --json` is complete offline.
pub fn build(stages: &[Stage], generated_at: String) -> Catalog {
    Catalog {
        track: "wasm".to_string(),
        generated_at,
        sections: sections()
            .iter()
                ...
                tests: s
                    ...
                    .map(|t| CatalogTest {
                        ...
                        tags: t
                            ...
                            .map(|x| x.to_string())
                            .collect(),
                        skip_on: t.skip_on.iter().map(|(r, _)| r.to_string()).collect(),
                    })
                    .collect(),
                examples: (s.examples)().iter().map(examples::render).collect(),
            })
            .collect(),
    }
}
...
pub fn list(stages: &[Stage], plan: &Path) {
    ...
    for st in stages {
        ...
        println!(
            "{tick} Stage {:02}  {:<48} {:<44} {} tests{ext_note}, {} examples",
            st.number,
            st.name,
            st.file_name(),
            st.tests.len(),
    ...
    let planned = 45usize.saturating_sub(stages.len());
    println!(
...
mod tests {
    ...
    fn catalog_matches_the_documented_schema() {
        ...
        assert!(first["examples"][0]["request_hex"]
            .as_str()
            .unwrap_or_default()
            .starts_with("0061736d"));
    }
