...
The original plan, kept because it is the record of what was intended and by whom, not
because it is current. Every stage list in here has since been superseded by the `PLAN.md`
inside each tester, which is the one `--list` actually reads; the execution sections describe
agent assignments that are finished.

Read it for the reasoning — why these tracks, what each tester's contract is, why the site
generates from the testers rather than the other way round. For what exists today, read the
[README](../README.md) and the CI runs it links to.
