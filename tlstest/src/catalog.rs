...
//! `sections[]`, `stages[]{number,slug,name,ext,file,hints[],tests[],examples[]}`.
...
pub struct CatalogTest {
    ...
    pub tags: Vec<String>,
    /// Servers this test is skipped for.
...
pub struct CatalogStage {
    ...
    /// Worked examples, with per-field annotations captured from the reference server.
                ...
                tests: s
                    ...
                    .map(|t| CatalogTest {
                        ...
                        tags: t
                            ...
                            .map(|x| x.to_string())
                            .collect(),
                        skip_on: t.skip_on.iter().map(|(s, _)| s.to_string()).collect(),
                    })
                    .collect(),
                examples: {
...
pub fn list(stages: &[Stage], plan: &Path) {
    ...
    for st in stages {
        ...
        println!(
            "{tick} Stage {:02}  {:<52} {:<42} {} tests{ext_note}, {} examples",
            st.number,
            st.name,
            st.file_name(),
            st.tests.len(),
    ...
    let examples: usize = stages.iter().map(|s| (s.examples)().len()).sum();
    println!(
        "\n{} stages implemented, {total} tests, {examples} examples (0 stages still planned)",
        stages.len()
    );
}
...
mod tests {
    ...
    fn catalog_matches_the_documented_schema() {
        ...
        assert!(first["examples"].is_array());
    }
