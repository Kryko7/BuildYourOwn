...
//!
//! The JSON schema is the one `shelltest` and `kafkatest` write — `target`, `validate`,
//! `stages[]`, `passed/failed/skipped/elapsed_ms` and per-test `status/ext/duration_ms/
//! failures/actual/failure_kind/notes` — so `byo` ingests a TLS report with the same parser
//! it uses for the other tracks. Nothing here is renamed.
...
pub struct Reporter {
    /// True when the run is a suite self-check against the reference server.
    pub validate: bool,
...
impl Reporter {
    ...
    fn failure_block(&self, f: &Failure) {
        ...
        for (title, bytes, marks) in f.hex_blocks() {
            self.section(&title);
            self.row(&crate::assert::hexdump(&bytes, &marks, 512));
        }
    }

    /// Print the server's captured output under a failing test.
    pub fn server_output(&self, text: &str) {
        if text.trim().is_empty() {
            return;
        }
        self.section("server output");
        self.row(text);
    }
    ...
    pub fn total(
        ...
        if failed > 0 && self.validate {
            self.out(&c(
                &format!(
                    "SUITE BUGS: the reference server '{target}' failed {failed} test(s). \
                     Fix the expectations, not the server:"
                ),
                Tint::Red,
            ));
            for (st, results) in all {
...
/// Top-level `--json` document; the schema `shelltest` and `kafkatest` also write.
#[derive(Serialize)]
    ...
    fn json_uses_target_and_counts_correctly() {
        ...
        let results = vec![
            TestResult {
                ...
                failure: None,
                skip_reason: None,
                notes: vec!["handshake took 4 ms".into()],
            },
            TestResult {
                name: "bad".into(),
                status: Status::Fail,
                ext: true,
                duration_ms: 2,
                failure: Some(Failure::new(FailureKind::ServerCrash, "boom")),
                skip_reason: None,
                notes: Vec::new(),
            },
        ];
        ...
        assert_eq!(v["stages"][0]["file"], "src/stages/s17_server_hello.rs");
