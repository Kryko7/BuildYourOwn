...
impl TestCase {
    ...
    fn validate(&self) -> Result<()> {
        ...
        let e = &self.expect;
        let nothing = e.stdout.is_none()
            && e.stderr.is_none()
            && e.terminal.is_none()
            && e.exit_code.is_none()
            && e.files.is_empty()
            && e.file_absent.is_empty();
        if nothing {
...
pub fn load_stage(path: &Path) -> Result<Stage> {
    ...
    let map = doc
        .as_mapping()
        .ok_or_else(|| anyhow!("{}: top level must be a mapping", path.display()))?;
    for k in map.keys() {
    ...
    let stage = map
        .get("stage")
        .and_then(|v| v.as_u64())
        .ok_or_else(|| anyhow!("{}: missing or non-integer 'stage'", path.display()))?
        as u32;
    let name = map
        .get("name")
        .and_then(|v| v.as_str())
    ...
    for (i, raw) in raw_tests.iter().enumerate() {
        ...
        let tc: TestCase = serde_yaml::from_value(raw.clone()).map_err(|e| {
            anyhow!(
                "{}: test '{tname}': {}",
                path.display(),
                clean_err(&e.to_string())
            )
        })?;
        tc.validate()
            .map_err(|e| anyhow!("{}: test '{tname}': {e}", path.display()))?;
